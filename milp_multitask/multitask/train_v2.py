...
from MIPDataset_configuration import GraphDataset as GraphDataset_configuration
from GAT import GATPolicy_multitask_v2 as GATPolicy
...
sample_files = [(os.path.join(args.instance_dir2,name), os.path.join(args.result_dir2,name)) for name in sample_names]
sample_files = sorted(sample_files)
...
train_data = GraphDataset_configuration(train_files)
train_loader2 = torch_geometric.loader.DataLoader(train_data, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, follow_batch=['x_vars', 'x_cons'])
valid_data = GraphDataset_configuration(valid_files)
valid_loader2 = torch_geometric.loader.DataLoader(valid_data, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, follow_batch=['x_vars', 'x_cons'])
...
def train(epoch, predict, data_loader1, data_loader2, optimizer1=None, optimizer2=None, weight_norm=1):
    ...
    while True:
        ...
        with torch.set_grad_enabled(optimizer2 is not None):
            ...
            embeddings = predict(batch2.x_cons,
                        batch2.edge_index_cons_to_vars,
                        batch2.edge_attr,
                        batch2.x_vars, 2,variable_features_batch=batch2.x_vars_batch, constraint_features_batch=batch2.x_cons_batch)

            anchor_positive = []
            anchor_negative = []
            positive_idx = []
            negative_idx = []
            total_sample = len(batch2)

            for i in range(len(batch2)):
                for j in range(len(batch2.pos_sample[i])):
                    anchor_positive.append(i)
                    positive_idx.append(total_sample)
                    tensor = torch.tensor(batch2.pos_sample[i][j]).unsqueeze(0)
                    embeddings = torch.cat([embeddings, tensor.to(DEVICE)])
                    total_sample += 1
                for j in range(len(batch2.neg_sample[i])):
                    anchor_negative.append(i)
                    negative_idx.append(total_sample)
                    tensor = torch.tensor(batch2.neg_sample[i][j]).unsqueeze(0)
                    embeddings = torch.cat([embeddings, tensor.to(DEVICE)])
                    total_sample += 1
        ...
        #     triplets = (torch.tensor(anchor_positive).to(DEVICE), torch.tensor(positive_idx).to(DEVICE), torch.tensor(anchor_negative).to(DEVICE), torch.tensor(negative_idx).to(DEVICE))
        #     loss3 = infoNCE_loss_function(embeddings, indices_tuple = triplets)
        #     if optimizer1 is not None:
        #         writer.add_scalar('train_loss3', loss3, epoch)
        #         optimizer1.zero_grad()
        #         loss3.backward()
        #         optimizer1.step()
        #     else:
        #         writer.add_scalar('valid_loss3', loss3, epoch)
        #     mean_loss3 += loss3
        ...
        #                 batch2.edge_index_cons_to_vars,
        #                 batch2.edge_attr,
        #                 batch2.x_vars, 4,variable_features_batch=batch2.x_vars_batch, constraint_features_batch=batch2.x_cons_batch)
        ...
        #     triplets = (torch.tensor(anchor_positive).to(DEVICE), torch.tensor(positive_idx).to(DEVICE), torch.tensor(anchor_negative).to(DEVICE), torch.tensor(negative_idx).to(DEVICE))
        #     loss4 = infoNCE_loss_function(embeddings, indices_tuple = triplets)

        #     if optimizer2 is not None:
        #         writer.add_scalar('train_loss4', loss4, epoch)
        #         optimizer2.zero_grad()
        #         loss4.backward()
        #         optimizer2.step()
        #     else:
        #         writer.add_scalar('valid_loss4', loss4, epoch)
        #     mean_loss4 += loss4
        ...
        #     triplets = (torch.tensor(anchor_positive).to(DEVICE), torch.tensor(positive_idx).to(DEVICE), torch.tensor(anchor_negative).to(DEVICE), torch.tensor(negative_idx).to(DEVICE))
        #     loss5 = infoNCE_loss_function(embeddings, indices_tuple = triplets)
        #     if optimizer1 is not None:
        #         writer.add_scalar('train_loss5', loss5, epoch)
        #         optimizer1.zero_grad()
        #         loss5.backward()
        #         optimizer1.step()
        #     else:
        #         writer.add_scalar('valid_loss1', loss5, epoch)
        #     mean_loss5 += loss5
        ...
        #                 batch2.edge_index_cons_to_vars,
        #                 batch2.edge_attr,
        #                 batch2.x_vars, 6,variable_features_batch=batch2.x_vars_batch, constraint_features_batch=batch2.x_cons_batch)
        ...
        #     triplets = (torch.tensor(anchor_positive).to(DEVICE), torch.tensor(positive_idx).to(DEVICE), torch.tensor(anchor_negative).to(DEVICE), torch.tensor(negative_idx).to(DEVICE))
        #     loss6 = infoNCE_loss_function(embeddings, indices_tuple = triplets)

        #     if optimizer2 is not None:
        #         writer.add_scalar('train_loss6', loss6, epoch)
        #         optimizer2.zero_grad()
        #         loss6.backward()
        #         optimizer2.step()
        #     else:
        #         writer.add_scalar('valid_loss6', loss6, epoch)
        #     mean_loss6 += loss6

    mean_loss1 = mean_loss1 / n_samples_processed1
    mean_loss2 = mean_loss2 / n_samples_processed2
    # mean_loss3 = mean_loss3 / n_samples_processed1
    # mean_loss4 = mean_loss4 / n_samples_processed2
    # mean_loss5 = mean_loss5 / n_samples_processed1
    # mean_loss6 = mean_loss6 / n_samples_processed2
