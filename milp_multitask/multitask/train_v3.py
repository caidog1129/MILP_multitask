...
from GAT import GATPolicy_multitask_v3 as GATPolicy
...
sample_files = [(os.path.join(args.instance_dir1,name).replace('.sol',''), os.path.join(args.result_dir1,name)) for name in sample_names]
...
train_loader1 = torch_geometric.loader.DataLoader(train_data, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS)
valid_data = GraphDataset_pas(valid_files, args)
valid_loader1 = torch_geometric.loader.DataLoader(valid_data, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS)
...
def train(epoch, predict, data_loader1, data_loader2, optimizer1=None, optimizer2=None, weight_norm=1):
    ...
    while True:
        ...
        with torch.set_grad_enabled(optimizer1 is not None):
            ...
            for i in range(solInd.shape[0]):#for in batch
                nvar = len(batch1.varInds[i][0][0])
                solStartInd = solEndInd
                solEndInd = solInd[i] * nvar + solStartInd
                valStartInd = valEndInd
                valEndInd = valEndInd + solInd[i]
                
                sols = batch1.solutions[solStartInd:solEndInd].reshape(-1, nvar)
                vals = batch1.objVals[valStartInd:valEndInd]
                ...
                negs = batch1.negsamples[negSampStartInd:negSampEndInd].reshape(-1, nvar)
                target_negs.append(negs)

            # Compute the logits (i.e. pre-softmax activations) according to the policy on the concatenated graphs
            batch1.constraint_features[torch.isinf(batch1.constraint_features)] = 10 #remove nan value
            #predict the binary distribution, BD
            BD = predict(
                batch1.constraint_features,
                batch1.edge_index,
                batch1.edge_attr,
                batch1.variable_features, 
                1
            )
            BD = BD.sigmoid()
    ...
    mean_loss = mean_loss1 + mean_loss2 / 3
