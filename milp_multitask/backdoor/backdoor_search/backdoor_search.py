...
def main(instance_dir,
    ...
    instance_mps_path = instance_dir
    instance_warmstart_path = None
    ...
    os.makedirs("datasets/" + exp_name + "/log/", exist_ok=True)
    os.makedirs("datasets/" + exp_name + "/BD/", exist_ok=True)
    instance_bkd_path = "datasets/" + exp_name + "/log/" + instance_dir.split("/")[-1]
    # instance_incb_path = '%s/incumbent' % instance_output_dir
    backdoor_list = "datasets/" + exp_name + "/BD/" + instance_dir.split("/")[-1]
...
if __name__ == '__main__':
    ...
    parser_main.add_argument("--config", type=str, default='backdoor_search/configs/config.ini')
